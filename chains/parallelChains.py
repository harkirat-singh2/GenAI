...
prompt1 =PromptTemplate(
  template='Generate Short and simple notes about the \n {text}',
  input_variables=["text"]
  )
prompt2 =PromptTemplate(
  template='Generate a five short question answers from the following \n {text}',
  input_variables=["text"]
  )

prompt3 = PromptTemplate(
  template='Merge the notes and quiz into a single document notes-> {notes} and quiz -> {quiz}',
